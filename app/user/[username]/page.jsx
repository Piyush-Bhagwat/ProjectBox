...
const ProfilePage = ({ params }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row p-8 lg:p-16 space-y-8 lg:space-y-0 lg:space-x-8">
      <div className="w-full lg:w-1/3 flex flex-col items-center bg-neutral-900 text-neutral-200 p-6 lg:p-8 rounded-lg shadow-lg flex-shrink-0 p-1 border border-gray-200 rounded-md border-gray-300">
        ...
        {isEditing ? (
          <div className="mt-4 w-full">
            ...
            />
            <div className="mt-4">
              <h3 className="text-lg font-semibold text-center">Social Links</h3>
              <div className="flex flex-col space-y-2 mt-2">
                {Object.keys(socialLinks).map((key) => (
                  <input
                    ...
                    placeholder={`Enter ${key.charAt(0).toUpperCase() + key.slice(1)} URL`}
                  />
                ))}
              </div>
            </div>
          ...
          <>
            <h2 className="text-2xl mt-4 font-semibold">{params.username}</h2>
            <div className="mt-6 w-full">
              <h3 className="text-lg font-semibold">Social Links</h3>
              <ul className="mt-2 list-none flex flex-col space-y-2">
                {Object.entries(savedSocialLinks).map(([key, url]) => (
                  url ? (
